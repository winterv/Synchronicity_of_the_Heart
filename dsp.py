...
class ExpFilter:
    ...
    def __init__(
        self,
        val: float | NDArray[Any] | list | tuple = 0.0,
        ...
        self.value: float | NDArray[Any] | list | tuple = val

    def update(self, value):
        if isinstance(self.value, (list, np.ndarray, tuple)):
            v = np.asarray(value, dtype=np.float64)
            prev = np.asarray(self.value, dtype=np.float64)
            alpha = v - prev
            alpha[alpha > 0.0] = self.alpha_rise
            alpha[alpha <= 0.0] = self.alpha_decay
            self.value = alpha * v + (1.0 - alpha) * prev
            return self.value
        v = float(value)
        alpha = self.alpha_rise if v > self.value else self.alpha_decay
        self.value = alpha * v + (1.0 - alpha) * float(self.value)
        return self.value
